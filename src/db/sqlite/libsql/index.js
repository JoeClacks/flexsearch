...
export default function libsqlHandle(client){
    ...
    // Tail of the FIFO chain. Every operation appends to it, so execution order
    // is call order regardless of who awaits what.
    let queue = Promise.resolve();
    // The open libsql transaction, when one is active.
    let tx = null;
    ...
    function enqueue(task){
        ...
        // Keep the chain alive after a failure, and stop an un-awaited
    ...
    function query(sql, params, callback, single){
        ...
        return enqueue(function(){
            return target().execute({ sql: sql, args: values }).then(
                function(result){
                    const rows = result.rows || [];
                    const out = single ? rows[0] : rows;
                    cb && cb(null, out);
                    return out;
                },
                settle(cb, sql)
    ...
    return {
        ...
        get(sql, params, callback){
            return query(sql, params, callback, true);
        },

        all(sql, params, callback){
            return query(sql, params, callback, false);
        },
        ...
        exec(sql, callback){
            ...
            if(BEGIN.test(sql)){
                return enqueue(function(){
                    // A nested BEGIN would throw in sqlite too; treat an
                    // already-open transaction as the one asked for.
            ...
            if(COMMIT.test(sql) || ROLLBACK.test(sql)){
                ...
                return enqueue(function(){
                    ...
                    return Promise.resolve(
                        rollback ? handle.rollback() : handle.commit()
                    ).then(function(){
                        cb && cb(null, []);
                    }, settle(cb, sql));
                });
            ...
            // executeMultiple takes a script and no parameters, matching exec;
            // execute takes exactly one statement. Pick by whether anything
            // follows a semicolon.
            const script = /;\s*\S/.test(sql.trim().replace(/;\s*$/, ""));

            return enqueue(function(){
                const t = target();
                const done = script && t.executeMultiple
                    ? t.executeMultiple(sql)
                    : t.execute(sql);
